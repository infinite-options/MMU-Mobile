...
const AccountSetup6Available = () => {
  ...
  const handleButton = (id, type) => {
    setFormData((prevFormData) => {
      const updatedArray = prevFormData[type].includes(id) ? prevFormData[type].filter((item) => item !== id) : [...prevFormData[type], id];
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.gridContainer}>
        {dates.map((date) => (
          <TouchableOpacity key={date} onPress={() => handleButton(date, "dates")} style={styles.dateButton}>
            <Text style={[styles.dateText, formData.dates.includes(date) && styles.selectedDateText]}>{date}</Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <Text style={styles.subHeaderText}>
        {" "}
        These availability slots are crucial to help you and potential matches make a date faster. {"\n"}
