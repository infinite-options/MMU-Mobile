...
const baseApiURL = "https://41c664jpz1.execute-api.us-west-1.amazonaws.com/dev";

export async function postUserData(formData) {
  try {
    const response = await fetch(`${baseApiURL}/userinfo`, {
      method: "PUT",
      body: formData,
    });
    if (!response.ok) {
...
export async function fetchUserInfo(user_uid) {
  try {
    const response = await fetch(`${baseApiURL}/userinfo` + "/" + user_uid);
