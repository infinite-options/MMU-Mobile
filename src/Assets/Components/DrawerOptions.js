...
const DrawerOptions = () => {
  ...
  return (
    <Modal visible={open} animationType='slide' transparent={true} onRequestClose={() => setOption("")}>
      <View style={styles.modalContainer}>
        {option && ["religion", "position", "nationality"].includes(option) ? (
          <View style={styles.textInputContainer}>
            ...
            <TextInput style={styles.textInput} value={textValue} onChangeText={setTextValue} placeholder={`Enter your ${option}`} autoFocus />
            <TouchableOpacity style={styles.saveButton} onPress={handleSaveText}>
              <Text style={styles.saveButtonText}>Save</Text>
            </TouchableOpacity>
          </View>
        ) : (
          option && <BodyScroll title={optionsData[option]?.title || "Select Option"} options={optionsData[option]?.options || []} />
        )}
      </View>
    </Modal>
  );
